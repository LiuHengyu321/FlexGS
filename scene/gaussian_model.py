...
from scene.deformation import Gumbel_Network
...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, args, is_gumbel=False):
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        if self.is_gumbel:
            self.gumbel_net = self.gumbel_net.to("cuda") 
    ...
    def training_setup(self, training_args):
        ...
        if self.is_gumbel:
            l = [
                ...
                {'params': self.gumbel_net.parameters(), 'lr': training_args.deformation_lr_init * self.spatial_lr_scale, "name": "gumbel"} 
            ]
        else:
        ...
        self.grid_scheduler_args = get_expon_lr_func(lr_init=training_args.grid_lr_init*self.spatial_lr_scale,
                                                    ...
                                                    lr_delay_mult=training_args.deformation_lr_delay_mult,
                                                    max_steps=training_args.position_lr_max_steps)    
        self.gumbel_scheduler_args = get_expon_lr_func(lr_init=training_args.deformation_lr_init*self.spatial_lr_scale,
                                                    lr_final=training_args.deformation_lr_final*self.spatial_lr_scale,
                                                    lr_delay_mult=training_args.deformation_lr_delay_mult,
                                                    max_steps=training_args.position_lr_max_steps)  
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if  "grid" in param_group["name"]:
                ...
                param_group['lr'] = lr
                # return lr
            elif param_group["name"] == "gumbel":
                lr = self.gumbel_scheduler_args(iteration)
                param_group['lr'] = lr
    ...
    def load_model(self, path):
        ...
        if os.path.exists(os.path.join(path, "gumbel_net.pth")):
            self.gumbel_net = torch.load(os.path.join(path, "gumbel_net.pth"), map_location="cuda")
            self.is_gumbel = True
    ...
    def save_deformation(self, path):
        ...
        if self.is_gumbel:
            torch.save(self.gumbel_net, os.path.join(path, "gumbel_net.pth"))
    ...
    def save_select(self, path, selected_mask, ratio):
        mkdir_p(os.path.dirname(path))
        ...
        norm_rotation = rotations_final.norm(p=2, dim=-1, keepdim=True)
        
        rotations_final = self.rotation_activation(rotations_final)
        opacity = self.opacity_activation(opacity_final)
        ...
        time = torch.tensor(ratio, dtype=torch.float32).to(select_means3D.device).repeat(select_means3D.shape[0],1)
        select_means3D, select_scales, select_rotations, select_opacity, select_shs = self._deformation(
            select_means3D, select_scales, 
            select_rotations, select_opacity, select_shs,
            time
        )
        
        select_scales = self.scaling_inverse_activation(select_scales)
        select_rotations = select_rotations * norm_rotation[selected_mask]
        
        xyz = self._xyz[selected_mask].detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        f_dc = self._features_dc[selected_mask].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        f_rest = self._features_rest[selected_mask].detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities = self._opacity[selected_mask].detach().cpu().numpy()
        scale = select_scales.detach().cpu().numpy()
        rotation = select_rotations.detach().cpu().numpy()
